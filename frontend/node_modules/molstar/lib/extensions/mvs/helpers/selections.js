...
export function getAtomRangesForRow(model, row, indices) {
    ...
    var hasAtomIds = isAnyDefined(row.atom_id, row.atom_index);
    var hasAtomFilter = isAnyDefined(row.label_atom_id, row.auth_atom_id, row.type_symbol);
    var hasResidueFilter = isAnyDefined(row.label_seq_id, row.auth_seq_id, row.pdbx_PDB_ins_code, row.beg_label_seq_id, row.end_label_seq_id, row.beg_auth_seq_id, row.end_auth_seq_id);
    var hasChainFilter = isAnyDefined(row.label_asym_id, row.auth_asym_id, row.label_entity_id);
    if (hasAtomIds) {
        var theAtom = getTheAtomForRow(model, row, indices);
    ...
    var qualifyingChains = getQualifyingChains(model, row, indices);
    if (!hasResidueFilter && !hasAtomFilter) {
        var chainOffsets = h.chainAtomSegments.offsets;
        var ranges_1 = AtomRanges.empty();
        for (var _i = 0, qualifyingChains_1 = qualifyingChains; _i < qualifyingChains_1.length; _i++) {
            var iChain = qualifyingChains_1[_i];
            AtomRanges.add(ranges_1, chainOffsets[iChain], chainOffsets[iChain + 1]);
        }
        return ranges_1;
    }
    var qualifyingResidues = getQualifyingResidues(model, row, indices, qualifyingChains);
    if (!hasAtomFilter) {
        var residueOffsets = h.residueAtomSegments.offsets;
        var ranges_2 = AtomRanges.empty();
        for (var _a = 0, qualifyingResidues_1 = qualifyingResidues; _a < qualifyingResidues_1.length; _a++) {
            var iRes = qualifyingResidues_1[_a];
            AtomRanges.add(ranges_2, residueOffsets[iRes], residueOffsets[iRes + 1]);
        }
        return ranges_2;
    }
    var qualifyingAtoms = getQualifyingAtoms(model, row, indices, qualifyingResidues);
    var ranges = AtomRanges.empty();
    for (var _b = 0, qualifyingAtoms_1 = qualifyingAtoms; _b < qualifyingAtoms_1.length; _b++) {
        var iAtom = qualifyingAtoms_1[_b];
...
export function getAtomRangesForRows(model, rows, indices) {
    if (Array.isArray(rows)) {
        return AtomRanges.union(rows.map(function (row) { return getAtomRangesForRow(model, row, indices); }));
    }
    else {
...
function getQualifyingChains(model, row, indices) {
    var _a, _b, _c;
    var _d = model.atomicHierarchy.chains, auth_asym_id = _d.auth_asym_id, label_entity_id = _d.label_entity_id, nChains = _d._rowCount;
    ...
    if (isDefined(row.auth_asym_id)) {
        if (result) {
            result = result.filter(function (i) { return auth_asym_id.value(i) === row.auth_asym_id; });
        }
        else {
    ...
    if (isDefined(row.label_entity_id)) {
        if (result) {
            result = result.filter(function (i) { return label_entity_id.value(i) === row.label_entity_id; });
        }
        else {
...
function getQualifyingResidues(model, row, indices, fromChains) {
    var _a;
    var _b = model.atomicHierarchy.residues, label_seq_id = _b.label_seq_id, auth_seq_id = _b.auth_seq_id, pdbx_PDB_ins_code = _b.pdbx_PDB_ins_code;
    var Present = Column.ValueKind.Present;
    var result = [];
    for (var _i = 0, fromChains_1 = fromChains; _i < fromChains_1.length; _i++) {
        var iChain = fromChains_1[_i];
        var residuesHere = undefined;
        if (isDefined(row.label_seq_id)) {
            var sorting = indices.residuesSortedByLabelSeqId.get(iChain);
            residuesHere = Sorting.getKeysWithValue(sorting, row.label_seq_id);
        }
        if (isDefined(row.auth_seq_id)) {
            if (residuesHere) {
                residuesHere = residuesHere.filter(function (i) { return auth_seq_id.valueKind(i) === Present && auth_seq_id.value(i) === row.auth_seq_id; });
            }
            else {
                var sorting = indices.residuesSortedByAuthSeqId.get(iChain);
                residuesHere = Sorting.getKeysWithValue(sorting, row.auth_seq_id);
            }
        }
        if (isDefined(row.pdbx_PDB_ins_code)) {
            if (residuesHere) {
                residuesHere = residuesHere.filter(function (i) { return pdbx_PDB_ins_code.value(i) === row.pdbx_PDB_ins_code; });
            }
            else {
        ...
        if (isDefined(row.beg_label_seq_id) || isDefined(row.end_label_seq_id)) {
            if (residuesHere) {
                if (isDefined(row.beg_label_seq_id)) {
                    residuesHere = residuesHere.filter(function (i) { return label_seq_id.valueKind(i) === Present && label_seq_id.value(i) >= row.beg_label_seq_id; });
                }
                if (isDefined(row.end_label_seq_id)) {
                    residuesHere = residuesHere.filter(function (i) { return label_seq_id.valueKind(i) === Present && label_seq_id.value(i) <= row.end_label_seq_id; });
                }
            }
            else {
                var sorting = indices.residuesSortedByLabelSeqId.get(iChain);
        ...
        if (isDefined(row.beg_auth_seq_id) || isDefined(row.end_auth_seq_id)) {
            if (residuesHere) {
                if (isDefined(row.beg_auth_seq_id)) {
                    residuesHere = residuesHere.filter(function (i) { return auth_seq_id.valueKind(i) === Present && auth_seq_id.value(i) >= row.beg_auth_seq_id; });
                }
                if (isDefined(row.end_auth_seq_id)) {
                    residuesHere = residuesHere.filter(function (i) { return auth_seq_id.valueKind(i) === Present && auth_seq_id.value(i) <= row.end_auth_seq_id; });
                }
            }
            else {
                var sorting = indices.residuesSortedByAuthSeqId.get(iChain);
        ...
        if (!residuesHere) {
            var _c = model.atomicHierarchy, residueAtomSegments = _c.residueAtomSegments, chainAtomSegments = _c.chainAtomSegments;
            var firstResidueForChain = residueAtomSegments.index[chainAtomSegments.offsets[iChain]];
            var firstResidueAfterChain = residueAtomSegments.index[chainAtomSegments.offsets[iChain + 1] - 1] + 1;
...
function getQualifyingAtoms(model, row, indices, fromResidues) {
    var _a = model.atomicHierarchy.atoms, label_atom_id = _a.label_atom_id, auth_atom_id = _a.auth_atom_id, type_symbol = _a.type_symbol;
    var residueAtomSegments_offsets = model.atomicHierarchy.residueAtomSegments.offsets;
    var result = [];
    for (var _i = 0, fromResidues_1 = fromResidues; _i < fromResidues_1.length; _i++) {
        var iRes = fromResidues_1[_i];
        var atomIdcs = range(residueAtomSegments_offsets[iRes], residueAtomSegments_offsets[iRes + 1]);
        if (isDefined(row.label_atom_id)) {
            filterInPlace(atomIdcs, function (iAtom) { return label_atom_id.value(iAtom) === row.label_atom_id; });
        }
        if (isDefined(row.auth_atom_id)) {
            filterInPlace(atomIdcs, function (iAtom) { return auth_atom_id.value(iAtom) === row.auth_atom_id; });
        }
        if (isDefined(row.type_symbol)) {
            filterInPlace(atomIdcs, function (iAtom) { var _a; return type_symbol.value(iAtom) === ((_a = row.type_symbol) === null || _a === void 0 ? void 0 : _a.toUpperCase()); });
        }
...
function getTheAtomForRow(model, row, indices) {
    ...
    if (isDefined(row.atom_id) && isDefined(row.atom_index)) {
        var a1 = indices.atomsById.get(row.atom_id);
        var a2 = indices.atomsByIndex.get(row.atom_index);
        if (a1 !== a2)
            return undefined;
        iAtom = a1;
    }
...
export function atomQualifies(model, iAtom, row) {
    var _a;
    var h = model.atomicHierarchy;
    var iChain = h.chainAtomSegments.index[iAtom];
    var label_asym_id = h.chains.label_asym_id.value(iChain);
    var auth_asym_id = h.chains.auth_asym_id.value(iChain);
    var label_entity_id = h.chains.label_entity_id.value(iChain);
    ...
    if (!matches(row.label_entity_id, label_entity_id))
        return false;
    var iRes = h.residueAtomSegments.index[iAtom];
    var label_seq_id = (h.residues.label_seq_id.valueKind(iRes) === Column.ValueKind.Present) ? h.residues.label_seq_id.value(iRes) : undefined;
    var auth_seq_id = (h.residues.auth_seq_id.valueKind(iRes) === Column.ValueKind.Present) ? h.residues.auth_seq_id.value(iRes) : undefined;
    var pdbx_PDB_ins_code = h.residues.pdbx_PDB_ins_code.value(iRes);
    ...
    if (!matchesRange(row.beg_auth_seq_id, row.end_auth_seq_id, auth_seq_id))
        return false;
    var label_atom_id = h.atoms.label_atom_id.value(iAtom);
    var auth_atom_id = h.atoms.auth_atom_id.value(iAtom);
    var type_symbol = h.atoms.type_symbol.value(iAtom);
    var atom_id = model.atomicConformation.atomId.value(iAtom);
    var atom_index = h.atomSourceIndex.value(iAtom);
...
export function rowToExpression(row) {
    var and = MS.core.logic.and;
    var _a = MS.core.rel, eq = _a.eq, gte = _a.gre, lte = _a.lte;
    var macromolecular = MS.struct.atomProperty.macromolecular;
    var propTests = {};
    if (isDefined(row.label_entity_id)) {
...
function unionExpression(expressions) {
    return MS.struct.combinator.merge(expressions.map(function (e) { return MS.struct.modifier.union([e]); }));
}
...
export function groupRows(rows) {
    ...
    for (var i = 0; i < rows.length; i++) {
        ...
        if (group_id === undefined) {
            ...
            var groupIndex = groupMap.get(group_id);
    ...
    var rowIndices = range(rows.length).sort(function (i, j) { return groups[i] - groups[j]; });
    var offsets = [];
    for (var i = 0; i < rows.length; i++) {
    ...
    return { count: offsets.length - 1, offsets: offsets, grouped: rowIndices };
}
