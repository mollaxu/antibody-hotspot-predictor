...
function invalidateSnapshotKey(plugin, key, currentId) {
    if (!key)
        return false;
    return plugin.managers.snapshot.state.entries.some(function (e) { return (!currentId || e.snapshot.id !== currentId) && e.key === key; });
}
function AddSnapshot(_a) {
    var parent = _a.parent;
    var _b = React.useState({ key: '', name: '', description: '' }), state = _b[0], setState = _b[1];
    var add = function () {
    ...
    var keyExists = invalidateSnapshotKey(parent.plugin, state.key);
...
var LocalStateSnapshotList = /** @class */ (function (_super) {
    ...
    function LocalStateSnapshotList() {
        var _this = _super !== null && _super.apply(this, arguments) || this;
        _this.state = { editingId: undefined };
        _this.edit = function (e) {
            var id = e.currentTarget.getAttribute('data-id');
            if (!id)
                return;
            var current = _this.state.editingId;
            _this.setState({ editingId: id === current ? undefined : id });
        };
        _this.doneEdit = function () { return _this.setState({ editingId: undefined }); };
    ...
    LocalStateSnapshotList.prototype.render = function () {
        ...
        this.plugin.managers.snapshot.state.entries.forEach(function (e) {
            var _a;
            items.push((0, jsx_runtime_1.jsxs)("li", { className: 'msp-flex-row', children: [(0, jsx_runtime_1.jsxs)(common_1.Button, { "data-id": e.snapshot.id, onClick: _this.apply, className: 'msp-no-overflow', children: [(0, jsx_runtime_1.jsxs)("span", { style: { fontWeight: e.snapshot.id === current ? 'bold' : void 0 }, children: [!!e.key && "[".concat(e.key, "] "), e.name || new Date(e.timestamp).toLocaleString()] }), " ", (0, jsx_runtime_1.jsx)("small", { children: "".concat(e.snapshot.durationInMs ? (0, now_1.formatTimespan)(e.snapshot.durationInMs, false) : '') })] }), (0, jsx_runtime_1.jsx)(common_1.IconButton, { svg: icons_1.TuneSvg, "data-id": e.snapshot.id, title: 'Edit', onClick: _this.edit, flex: '28px' }), (0, jsx_runtime_1.jsx)(common_1.IconButton, { svg: icons_1.ArrowUpwardSvg, "data-id": e.snapshot.id, title: 'Move Up', onClick: _this.moveUp, flex: '20px' }), (0, jsx_runtime_1.jsx)(common_1.IconButton, { svg: icons_1.ArrowDownwardSvg, "data-id": e.snapshot.id, title: 'Move Down', onClick: _this.moveDown, flex: '20px' }), (0, jsx_runtime_1.jsx)(common_1.IconButton, { svg: icons_1.SwapHorizSvg, "data-id": e.snapshot.id, title: 'Replace', onClick: _this.replace, flex: '20px' }), (0, jsx_runtime_1.jsx)(common_1.IconButton, { svg: icons_1.DeleteOutlinedSvg, "data-id": e.snapshot.id, title: 'Remove', onClick: _this.remove, flex: '20px' })] }, e.snapshot.id));
            if (_this.state.editingId === e.snapshot.id) {
                items.push((0, jsx_runtime_1.jsx)(EditSnapshot, { entry: e, plugin: _this.plugin, done: _this.doneEdit }, "".concat(e.snapshot.id, "-edit")));
            }
...
function EditSnapshotParams(_a) {
    var state = _a.state, setState = _a.setState, apply = _a.apply;
    var keyRef = React.useRef();
    var descRef = React.useRef();
    var _b = React.useState(false), showKeyHelp = _b[0], setShowKeyHelp = _b[1];
    return (0, jsx_runtime_1.jsxs)(jsx_runtime_1.Fragment, { children: [(0, jsx_runtime_1.jsx)(common_1.ControlRow, { label: 'Name', control: (0, jsx_runtime_1.jsx)("input", { type: 'text', value: state.name, placeholder: 'Name', onChange: function (e) { return setState(tslib_1.__assign(tslib_1.__assign({}, state), { name: e.target.value })); }, onKeyUp: function (e) {
                        var _a;
                        if (e.key === 'Enter')
                            (_a = keyRef.current) === null || _a === void 0 ? void 0 : _a.focus();
                    } }) }), (0, jsx_runtime_1.jsx)(common_1.ControlRow, { label: (0, jsx_runtime_1.jsxs)(jsx_runtime_1.Fragment, { children: ["Key", (0, jsx_runtime_1.jsx)(parameters_1.ToggleParamHelpButton, { show: showKeyHelp, toggle: function () { return setShowKeyHelp(function (prev) { return !prev; }); } })] }), control: (0, jsx_runtime_1.jsx)("input", { type: 'text', ref: keyRef, value: state.key, placeholder: 'Key (optional)', onChange: function (e) { return setState(tslib_1.__assign(tslib_1.__assign({}, state), { key: e.target.value })); }, onKeyUp: function (e) {
                        var _a;
                        if (e.key === 'Enter')
                    ...
                    placeholder: 'Markdown Description\n\n- Use [title](#key) to link to a snapshot', className: 'msp-form-control', value: state.description, onChange: function (e) { return setState(tslib_1.__assign(tslib_1.__assign({}, state), { description: e.target.value })); }, onKeyUp: function (e) {
                        if (e.key === 'Enter' && e.ctrlKey)
                            apply(state);
                    } }) })] });
}
function EditSnapshot(_a) {
    var _b, _c, _d;
    var entry = _a.entry, plugin = _a.plugin, done = _a.done;
    var _e = React.useState({ key: (_b = entry.key) !== null && _b !== void 0 ? _b : '', name: (_c = entry.name) !== null && _c !== void 0 ? _c : '', description: (_d = entry.description) !== null && _d !== void 0 ? _d : '' }), state = _e[0], setState = _e[1];
    var apply = function () {
        plugin.managers.snapshot.update(entry, state);
        done();
    };
    var keyExists = invalidateSnapshotKey(plugin, state.key, entry.snapshot.id);
