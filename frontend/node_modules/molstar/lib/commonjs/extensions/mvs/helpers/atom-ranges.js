...
exports.AtomRanges = {
    ...
    add: function (ranges, from, to) {
        var n = exports.AtomRanges.count(ranges);
        if (n > 0) {
    ...
    union: function (ranges) {
        var concat = exports.AtomRanges.empty();
        for (var _i = 0, ranges_1 = ranges; _i < ranges_1.length; _i++) {
            var r = ranges_1[_i];
            (0, array_1.arrayExtend)(concat.from, r.from);
            (0, array_1.arrayExtend)(concat.to, r.to);
        }
        var indices = (0, array_1.range)(concat.from.length).sort(function (i, j) { return concat.from[i] - concat.from[j]; }); // sort by start of range
        var result = exports.AtomRanges.empty();
        var last = -1;
    ...
    selectAtomsInRanges: function (atoms, ranges, out, outFirstAtomIndex) {
        ...
        var nRanges = exports.AtomRanges.count(ranges);
        if (nAtoms <= nRanges) {
            // Implementation 1 (more efficient when there are fewer atoms)
            var iRange = int_1.SortedArray.findPredecessorIndex(int_1.SortedArray.ofSortedArray(ranges.to), atoms[0] + 1);
            ...
            for (var iRange = 0; iRange < nRanges; iRange++) {
                ...
                for (var iAtom = int_1.SortedArray.findPredecessorIndex(atoms, from); iAtom < nAtoms; iAtom++) {
                    var a = atoms[iAtom];
                    if (a < to) {
                        out.push(a);
