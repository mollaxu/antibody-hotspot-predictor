      ...
      <div className="w-full max-w-6xl px-6 py-8">
        ...
        <div className="grid gap-6 md:grid-cols-[minmax(0,1.6fr)_minmax(0,1fr)] items-start">
          ...
          <div className="space-y-6">
            ...
            <form
              onSubmit={handleScan}
              className="space-y-4 rounded-2xl bg-[#292929] px-5 py-5"
            >
              ...
              <div className="flex flex-wrap items-center gap-3 text-xs">
                <label className="inline-flex items-center gap-2 rounded-lg bg-[#363636] px-3 py-2 cursor-pointer hover:bg-[#404040] transition-colors">
                  <span className="text-slate-100">上传 PDB 文件（可选）</span>
                  <input
                    type="file"
                    accept=".pdb,.ent,.cif"
                    className="hidden"
                ...
                <button
                  type="button"
                  className="ml-auto md:hidden text-[11px] text-slate-300 underline"
                  onClick={() => setShow3DMobile((v) => !v)}
                >
                  {show3DMobile ? '收起 3D 视图' : '查看 3D'}
                </button>
              </div>
            ...
            {show3DMobile && (
              <div className="mt-4 md:hidden">
                <ProteinViewer pdbUrl={pdbUrl} selectedResidue={selectedResidue} />
              </div>
            )}
          </div>
